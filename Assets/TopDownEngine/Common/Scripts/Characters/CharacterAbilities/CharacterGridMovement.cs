...
{	
	...
	{
		public static Action<GridDirections> OnGridMovementEvent;
		...
		{
			if ((_currentDirection != GridDirections.None) && (CurrentSpeed < MaximumSpeed * MaximumSpeedMultiplier * EnergyToUse))
			{
				CurrentSpeed = CurrentSpeed + Acceleration * AccelerationMultiplier * EnergyToUse * EnergyToUse * Time.deltaTime;
		...
		protected virtual void HandleMovement()
		{
			...
			if (!_movingToNextGridUnit)
			{
				...
				if (_bufferedDirection == GridDirections.None)
				{
					...
					GridManager.Instance.SetLastPosition(gameObject, GridManager.Instance.WorldToCellCoordinates(_endWorldPosition));
					GridManager.Instance.SetNextPosition(gameObject, GridManager.Instance.WorldToCellCoordinates(_endWorldPosition));
					
					if (_energy > 0 && _currentDirection == GridDirections.None)
					{
						_currentDirection = _energyDirection;
					}
					else
					{
			...
			// 	
			// 	if (_movingToNextGridUnit && distance == 0)
			// 	{
			// 		_lastPosition = new Vector3(-1, -1, -1);
			// 		_movingToNextGridUnit = false;
			// 		_currentDirection = GridDirections.None;
			// 		_bufferedDirection = GridDirections.None;
			// 		_agentMoving = false;
		...
		{
			TargetGridPosition = CurrentCellCoordinates + (1 * ConvertDirectionToVector3Int(_currentDirection));
